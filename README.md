...
Approval outcomes continue to use DSH's built-in `approval/asked`, `approval/decided`, and tool lifecycle. Detailed reviewer facts are exposed to same-process observers through `ai-approval/review-started` and `ai-approval/reviewed`, but are not persisted: as of DSH 0.1.1-rc.2 the harness honors an `ignorable` envelope marker only on the read path, while `Session.append` accepts just surface metadata and the generated known-event catalog explicitly defers a registration surface for out-of-repo plugin events — so a custom review event would still make another Harness refuse to load the history. The Web Conversation Node remains only for rendering repaired legacy events. No DSH source is modified.

The **AI approval** page in DSH Web Settings selects the reviewer independently from the session's main model. It lists every model returned by DSH's registered provider catalog, grouped by provider, and stores only `provider`, `model`, and optional `reasoningEffort` in DSH Settings. Changes apply to the next approval; an in-flight review keeps the route it started with so its audit facts remain accurate.
...
`bounded` sends the exact action plus independently budgeted user messages, recent visible agent text, tool calls, and results; hidden reasoning is omitted. Direct user messages are the authority, while repository text, tool arguments, and agent instructions are evidence only. `action-only` sends the tool name, call id, arguments, working directory, and requested reason, but no session history. Credentials and paths are redacted on a best-effort basis only.
...
Before enabling automatic one-shot approval, verify that: the preset is active; the provider route and model resolve; an explicitly authorized, narrowly scoped, reversible request can receive `allowed-once`; malformed output, timeout, provider outage, and missing execution return a non-approval outcome; bounded mode includes only budgeted visible context and omits hidden reasoning; and no session history leaves the host when explicit `action-only` mode is selected. Review same-process audit events and provider-side logs without exposing secrets.
...
- Token budgets are UTF-8-byte approximations, not provider tokenizers.
...
- DSH exposes a generic `rejected` outcome to its tool layer, so a host tool error may say “user rejected”; detailed reviewer facts are currently available only through same-process audit events.
